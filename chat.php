...
<head>
	...
	.msgarrfrom {
		border-bottom: 8px solid #F5B7B1;
		float: right;
		margin-right: 45px;
	}
	.msgsentby {
...
function checkcookie() {		//check if username is set
		...
		xmlhttp.onreadystatechange=function() {
			if (xmlhttp.readyState==4 && xmlhttp.status==200) {
				...
				for (var i = 0; i < rl; i++) {
					...
					if (item[1] != undefined) {
						if (item[0] == username) {	//blue message by user
						...
						} else {	//grey messsages from someone else
...
function sendmsg() {
	var message = msginput.value;	//value of textbox
	if (message != "") {	//if message is blank - text box wont send blank messages
		// alert(msgarea.innerHTML)
		// alert(getcookie("messengerUname"))
		var username = getcookie("messengerUname");
		var xmlhttp=new XMLHttpRequest();		//ajax request
		xmlhttp.onreadystatechange=function() {		//anon function
			if (xmlhttp.readyState==4 && xmlhttp.status==200) {
				...
				msginput.value = ""; 	//blank textbox after message sent
			}
		}
	      xmlhttp.open("GET","updateMessages.php?username=" + username + "&message=" + message,true);		//send request
	      xmlhttp.send();
  	}
}
setInterval(function(){ update() }, 2500);	//refresh 2.5s
</script>
</body>
</html>